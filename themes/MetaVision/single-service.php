...
if (is_single() && !is_product()) {
    ...
    while (have_posts()) :
        ...
        <section class="row row-gap-4 h-100 justify-content-lg-between justify-content-lg-center align-content-start">
            <div class="bg-secondary solution-header position-relative <?= get_field('background_size') ?? ''; ?> bg-norepeat"
                 style="background-image: url('<?= get_field('background')['url'] ?? ''; ?>'); ">
                <div class="col-lg-4 col-12 h-75 d-flex flex-column align-items-start justify-content-lg-center justify-content-start pt-2 pt-lg-0 <?= $cur_lan == 'en' ? 'pe-5' : 'ps-5' ?>">
                ...
                <div style="height: 400px"
                    ...
                    <?php if (get_the_post_thumbnail_url()) { ?>
                        <img data-aos="fade-down" src="<?php echo get_the_post_thumbnail_url() ?>"
                             height="350"
                             class="object-fit-contain bg-white w-100"
                             alt="<?php the_title(); ?>">
                    <?php } ?>
                </div>
            </div>
                    ...
                    if ($capabilities):
                        ...
                        <div class="d-flex gap-3 flex-wrap align-items-center justify-content-center">
                            <?php foreach ($capabilities as $index => $capability): ?>
                                <img data-aos-offset="-500" data-aos="zoom-in" data-aos-delay="<?= $index * 5; ?>0"
                                     class="p-2 border borer-dark rounded-1 object-fit-cover" width="70" height="70"
                                     src="<?= $capability['url'] ?>" alt="<?= $capability['title'] ?>"
                                     title="<?= $capability['title'] ?>">
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                </div>
                <div class="px-lg-5 pt-3 pt-lg-0 overflow-hidden">
